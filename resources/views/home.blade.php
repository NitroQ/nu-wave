    ...
    <div class="col-lg-12">
        <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
            ...
            <div class="carousel-item active">
                <img src="/images/nu-manila.png" class="d-block img-cover" alt="...">
                </div>
            @else
                @php $r = 0; @endphp
                @foreach ($carousel as $f)
                <div class="carousel-item {{ $r == 0 ? 'active' : ''}}">
                <img src="/uploads/carousel/{{ $f->image }}" class="d-block img-cover" alt="...">
                </div>
                @php $r++; @endphp
                @endforeach
        ...
        @foreach ($articles as $a)
            ...
            <div class="col-lg-4">
                <div class="card alert-card">
                    <div class="card-body">
                        <img src="/uploads/articles/{{ $a->image or 'placeholder.png' }}" class="img-fluid" style=" width: 100%;height: 320px; object-fit: cover;">
                        <small class="date-sm">{{ $a->date != null ? \Carbon\Carbon::parse($a->date)->format('F j, Y') : $a->created_at->format('F j, Y') }}</small>
                        <div class="w-100 text-start my-3">
                            <div style="height: 25px">
                                <h5 class="fw-bold line-clamp">{{ mb_strimwidth($a->title, 0, 52, '...') }}</h5>
                            </div>
                            <p class="line-clamp mt-3" style="height: 310px; text-align:justify">{{ mb_strimwidth(strip_tags($a->description), 0, 900, '...') }}</p>
                            <a href="{{ route('article-view' , [$a->id]) }}" class="text-decoration-none color-blue fw-bolder">Read Details <i class="fas fa-chevron-circle-right ms-1"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @else
                        <div class="col-6 {{ $i>=3 ? 'mt-3': '' }}">
                            <div class="card alert-card">
                                <div class="card-body">
                                    <img src="/uploads/articles/{{ $a->image or 'placeholder.png' }}" class="img-fluid" style=" width: 100%;height: 200px; object-fit: cover;">
                                    <small class="date-sm">{{ $a->date != null ? \Carbon\Carbon::parse($a->date)->format('F j, Y') : $a->created_at->format('F j, Y') }}</small>
                                    <div class="w-100 text-start my-2">
                                        <div style="height: 10px;">
                                            <small class="fw-bold line-clamp">{{ mb_strimwidth($a->title, 0, 52, '...') }}</small>
                                        </div>
                                        <small class="line-clamp mt-lg-3 mt-5">{{ mb_strimwidth(strip_tags($a->description), 0, 250, '...') }}</small>
                                        <a href="{{ route('article-view' , [$a->id]) }}" class="text-decoration-none color-blue fw-bold"><small>Read Details <i class="fas fa-chevron-circle-right ms-1"></i></small></a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<div class="container-fluid mt-5 pt-5">
    ...
    <div class="row justify-content-center">
        @foreach ($products as $b)
            <div class="col-lg-3">
                <div class="card alert-card">
                    <a href="{{ route('products-view' , [$b->id]) }}" class="text-decoration-none"> <div class="card-body">
                        <img src="/uploads/products/{{ $b->image or 'placeholder.png' }}" class="card-img prod-img">
                        <div class="w-100 text-center mt-3">
                            <div style="height: 60px">
                                <h5 class="fw-bold line-clamp text-dark">{{ $b->title }}</h5>
                            </div>
                        </div>
                    </div>
                </div></a>
                ...
                <div class="card-body">
                    ...
                    <div class="row justify-content-center g-5">
                        <div class="col-lg-4 px-2">
                            <img src="/uploads/innovators/{{ $c->image or 'placeholder.png' }}" class="img-fluid rounded" style=" width: 100%;height: 500px; object-fit: cover;">
                        </div>
                        <div class="col-lg-6">
                            <div class="w-100 text-start my-3">
                                <div style="height: 60px">
                                    <h3 class="fw-bold line-clamp">{{ $c->iname }}</h3>
                                    <h5 class="fw-bold fst-italic">{{ $c->position}}</h5>
                                </div>
                                <p class="mt-5" style="text-align: justify">{!! strip_tags( mb_strimwidth($c->about, 0, 900, '...')) !!}</p>
                                <a href="{{ route('innovator-view' , [$c->id]) }}" class="text-decoration-none color-blue fw-bolder">Read Details <i class="fas fa-chevron-circle-right ms-1"></i></a>
                            </div>
                        </div>
                    </div>
                    @endforeach
...
<div class="container-fluid my-5 pt-5">
    <div class="row justify-content-center">
        <div class="col-lg-9 text-start">
            <h1 class=" mt-5 h-b">Partners</h1>
            <p>The people who made this possible..</p>
        </div>
    </div>
    <div class="row justify-content-center align-items-center g-5">
        <div class="col-lg-3">
            <img src="/images/centie.png" alt="" class="img-fluid">
        </div>
        <div class="col-lg-3">
            <img src="/images/NUCCIT.png" alt="" class="img-fluid">
        </div>
        <div class="col-lg-3">
            <img src="/images/wizards.png" alt="" class="img-fluid">
        </div>
