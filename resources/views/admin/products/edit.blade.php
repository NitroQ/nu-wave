...
<div class="row">
	<div class="col-lg-12">
        ...
        <form method="POST" action="{{ route('product.update', [$product->id]) }}" enctype="multipart/form-data" class="row">
            {{-- {!! csrf_field() !!} --}}   <input type="hidden" name="_token" value="{{ csrf_token() }}">

            <div class="form-group col-lg-6">
                <label for="title">Title</label>
                <input type="text" class="form-control" name="title" placeholder="Enter Title" value="{{ $product->date }}">
                <span style="color: #FC1838">{{$errors->first('title')}}</span>
            </div>

            <div class="form-group col-lg-6">
                <label for="tag">Product Tags</label>
                <input type="text" class="form-control" name="tag" placeholder="Enter Hashtags" value="{{ $product->tag }}">
            ...
            <div class="form-group col-lg-3">
                ...
                <select class="form-control" name="category" id="supplier-list">
                    <option value="Information Technology" {{ $product->category == "Information Technology" ? 'selected' : ''}}>IT</option>
                    <option value="Construction" {{ $product->category == "Construction" ? 'selected' : ''}}>Construction</option>
                    <option value="Home/Appliances"{{ $product->category == "Home/Appliances" ? 'selected' : ''}} >Home/Appliances</option>
                    <option value="Personal" {{ $product->category == "Personal" ? 'selected' : ''}}>Personal Items</option>
                </select>
            ...
            <div class="form-group col-lg-3">
                ...
                <input type="text" class="form-control" name="shop" placeholder="Enter Link" value="{{ $product->shop }}">
			...
			<div class="form-group col-lg-12">
				...
				<textarea name="abstract" class="form-control" id="abstract">{{ $product->abstract }}</textarea>
                <span style="color: #FC1838">{{$errors->first('abstract')}}</span>
			</div>
            <div class="form-group col-lg-12">
				<label for="purpose">Purpose</label>
				<textarea name="purpose" class="form-control" id="purpose">{{ $product->purpose }}</textarea>
